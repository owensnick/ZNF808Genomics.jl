...
function ips_comparison(deresults, design_ips_full, ncounts)
    ...
    @with stack(@subset(deg_ips, :Stage .!= "S4"), [:log2FoldChange, :ips_l2fc], [:Gene, :Stage, :dsig]) groupedboxplot!(:Stage, :value,
        group=(ifelse.(:dsig .== 1, "Activated", "Repressed"), ifelse.(:variable .== "log2FoldChange", "ZNF808 del", "iPSC")), outliers=false, c=^([:orange :darkorange2 :steelblue1 :steelblue]))
    ...
    plot(pb, phs..., layout=lt, size=(700*1.1, 300*1.1), fontfamily="helvetica")
